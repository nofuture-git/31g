...
namespace NoFuture.Util.Core.Math.Matrix.Tests
{
    ...
    public class MatrixTests
    {
        ...
        public void TestAreEqual()
        {
            ...
            typicalMatrix = MatrixOps.RandomMatrix(100, 100);
            var copy = typicalMatrix.Copy();

            //two different ref's on heap
            Assert.IsFalse(typicalMatrix.Equals(copy));

            Assert.IsTrue(MatrixOps.AreEqual(typicalMatrix, copy));
        }

        [Test]
        public void TestAreEqualVectors()
        {
            var typicalVector = new double[] {4, 5, 6, 7, 8, 9};
            var copy = typicalVector.Copy();
            Assert.IsTrue(MatrixOps.AreEqual(copy, typicalVector));
            ...
            var testInput = new[,]
{
        ...
        public void TestInverse()
        {
            ...
            //from python's numpy.linalg.inv(a)
            expect = new[,]
            {
            ...
            testResult = testInput.Inverse();
            Assert.IsTrue(MatrixOps.AreEqual(expect, testResult, 0.00001));
        }
        ...
        {
            var testInput = MatrixOps.RandomMatrix(100,100);
        ...
        [Test]
        public void TestShuffleRows_WithSync()
        {
            var testInput = MatrixOps.RandomMatrix(10,10);
            var testCtrl = testInput.Copy();
            var testSync = MatrixOps.RandomMatrix(10, 1);
            var testCtrlSync = testSync.Copy();
            var testResult = testInput.ShuffleRows(testSync);

            for (var i = 0; i < testCtrl.CountOfRows(); i++)
            {
                var testRow = testCtrl.GetRow(i);
                var testMatchRowIndex = -1;
                for (var ii = 0; ii < testResult.CountOfRows(); ii++)
                {
                    if (MatrixOps.AreEqual(testRow, testResult.GetRow(ii)))
                    {
                        testMatchRowIndex = ii;
                        break;
                    }
                }
                Assert.IsTrue(testMatchRowIndex >= 0);

                //now know that i is-to testCtrl what testMatchRowIndex is-to testResult
                var syncValueFromCtrl = testCtrlSync[i,0];
                var syncValueFromTest = testSync[testMatchRowIndex, 0];

                Assert.AreEqual(syncValueFromCtrl, syncValueFromTest);
            ...
            var y = new double[]
{
        ...
        [Test]
        public void TestStochasticLinearGradientDescent()
        {
            var y = new double[]
{
                12.2907482,
