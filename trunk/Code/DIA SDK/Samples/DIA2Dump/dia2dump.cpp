...
bool DumpAllMods(IDiaSymbol *pGlobal)
{
  ...
  ULONG celt = 0;
  ULONG iMod = 1;
  LONG pEnumSymbolsLength;
  LONG atCount = 0;

  if(pEnumSymbols->get_Count(&pEnumSymbolsLength) != S_OK) {
	  pEnumSymbolsLength = -1;
  }
  ...
  while (SUCCEEDED(pEnumSymbols->Next(1, &pCompiland, &celt)) && (celt == 1)) {
	...
	atCount = atCount + 1;
	wprintf(L"{\"id\" : \"%04X\", \"name\" : \"%s\"}\n", iMod++, bstrName);
	if(atCount != pEnumSymbolsLength) {
		wprintf(L",");
	}
...
bool DumpAllSymbols(IDiaSymbol *pGlobal)
{
  ...
  while (SUCCEEDED(pEnumSymbols->Next(1, &pCompiland, &celt)) && (celt == 1)) {
    ...
    if (pCompiland->get_name(&bstrName) != S_OK) {
      wprintf(L"{\"moduleName\": \"unknown\"");
    }

    else {
      wprintf(L"{\"moduleName\": \"%s\"", bstrName);
    ...
    if (SUCCEEDED(pCompiland->findChildren(SymTagNull, NULL, nsNone, &pEnumChildren))) {
	  ...
	  LONG pEnumChildrenLength;
	  LONG childrenAtCount = 0;
	  
	  if(pEnumChildren->get_Count(&pEnumChildrenLength) != S_OK) {
		  pEnumChildrenLength = -1;
	  }
	  wprintf(L", \n  \"moduleSymbols\": [");

      while (SUCCEEDED(pEnumChildren->Next(1, &pSymbol, &celtChildren)) && (celtChildren == 1)) {
		wprintf(L"\n");
		childrenAtCount = childrenAtCount + 1;
        PrintSymbol(pSymbol, 6, pEnumChildrenLength == childrenAtCount);
        pSymbol->Release();
      }//end while
...
bool DumpAllTypes(IDiaSymbol *pGlobal)
{
  ...
  bool returnVal =  DumpAllUDTs(pGlobal) || DumpAllEnums(pGlobal) || DumpAllTypedefs(pGlobal);
...
bool DumpAllFiles(IDiaSession *pSession, IDiaSymbol *pGlobal)
{
  ...
  while (SUCCEEDED(pEnumSymbols->Next(1, &pCompiland, &celt)) && (celt == 1)) {
    ...
    if (SUCCEEDED(pSession->findFile(pCompiland, NULL, nsNone, &pEnumSourceFiles))) {
	  ...
	  LONG innerEnumLength;
	  LONG innerCount = 0;

	  if(pEnumSourceFiles->get_Count(&innerEnumLength) != S_OK) {
		  innerEnumLength = -1;
	  }
...
bool DumpAllLines(IDiaSession *pSession, IDiaSymbol *pGlobal)
{
  ...
  while (SUCCEEDED(pEnumSymbols->Next(1, &pCompiland, &celt)) && (celt == 1)) {
    ...
    if (pCompiland->get_name(&bstrName) == S_OK) {
       wprintf(L"{\"moduleName\": \"%s\", \"moduleSymbols\": [\n", bstrName);

       SysFreeString(bstrName);
    }
	else {
		wprintf(L"{\"moduleName\": \"__unknown\", \"moduleSymbols\": [\n");
	}
    ...
    if (SUCCEEDED(pCompiland->findChildren(SymTagFunction, NULL, nsNone, &pEnumFunction))) {
	  ...
	  if(pEnumFunction->get_Count(&innerEnumLength) != S_OK) {
		  innerEnumLength = -1;
	  }
...
bool DumpAllSecContribs(IDiaSession *pSession)
{
  ...
  ULONG celt = 0;
  LONG pEnumSecContribsLength;
  LONG atCount = 0;

  if(pEnumSecContribs->get_Count(&pEnumSecContribsLength) != S_OK) {
	  pEnumSecContribsLength = -1;
  }
  ...
  while (SUCCEEDED(pEnumSecContribs->Next(1, &pSecContrib, &celt)) && (celt == 1)) {
	...
	if(atCount != pEnumSecContribsLength) {
		wprintf(L",");
	}
...
bool DumpAllDebugStreams(IDiaSession *pSession)
{
  ...
  ULONG celt = 0;
  LONG enumStreamsLength;
  LONG atCount = 0;

  if(pEnumStreams->get_Count(&enumStreamsLength) != S_OK) {
	enumStreamsLength = -1;
  }
...
bool DumpCompiland(IDiaSymbol *pGlobal, const wchar_t *szCompName)
{
  ...
  while (SUCCEEDED(pEnumSymbols->Next(1, &pCompiland, &celt)) && (celt == 1)) {
    ...
    if (SUCCEEDED(pCompiland->findChildren(SymTagNull, NULL, nsNone, &pEnumChildren))) {
	  ...
	  if(pEnumChildren->get_Count(&innerEnumLength) != S_OK) {
		  innerEnumLength = -1;
	  }

      while (SUCCEEDED(pEnumChildren->Next(1, &pSymbol, &celt_)) && (celt_ == 1)) {
		innerCount = innerCount + 1;
		PrintLines(g_pDiaSession, pSymbol);
	    if(innerCount != innerEnumLength) {
		  wprintf(L",");
	    }
...
bool DumpSymbolWithChildren(IDiaSymbol *pSymbol, const wchar_t *szChildname)
{
  if (szChildname != NULL) {
    ...
    LONG pEnumSymbolsLength;
    LONG atCount = 0;
    if(pEnumSyms->get_Count(&pEnumSymbolsLength) != S_OK) {
	  pEnumSymbolsLength = -1;
    }
...
bool DumpLinesForSourceFile(IDiaSession *pSession, const wchar_t *szFileName, DWORD dwLine)
{
  ...
  LONG pEnumSymbolsLength;
  LONG atCount = 0;
  if(pEnumSrcFiles->get_Count(&pEnumSymbolsLength) != S_OK) {
	  pEnumSymbolsLength = -1;
  }
  ...
  while (SUCCEEDED(pEnumSrcFiles->Next(1, &pSrcFile, &celt)) && (celt == 1)) {
    ...
    if (pSrcFile->get_compilands(&pEnumCompilands) == S_OK) {
      ...
      celt = 0;
	  if(pEnumCompilands->get_Count(&innerEnumLength) != S_OK) {
		innerEnumLength = -1;
	  }
...
bool DumpMapToSrc(IDiaSession *pSession, DWORD dwRVA)
{
  ...
  for (; SUCCEEDED(pEnumStreams->Next(1, &pStream, &celt)) && (celt == 1); pStream = NULL) {
    ...
    if (bstrName && wcscmp(bstrName, L"OMAPTO") == 0) {
      ...
      wprintf(L"\"imgRelativeVirtualAddress\": \"%08X\", \"sourceRelativeVirtualAddress\": \"%08X\", \n", dwRVA, dwRVATo);
	  wprintf(L"\"RelatedOmapEntries\": {\"imgRelativeVirtualAddressFrom\": \"%08X\", \"imgRelativeVirtualAddressTo\": \"%08X\"},\n", datasav.dwRVA, datasav.dwRVATo);

      i = 0;

      do {
        wprintf(L"{\"relativeVirtualAddressFrom\": \"%08X\", \"relativeVirtualAddressTo\": \"%08X\"},\n", data.dwRVA, data.dwRVATo);
      }
